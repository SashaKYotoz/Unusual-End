		...
		if ((world.getBlockState(BlockPos.containing(x, y - 1, z))).is(BlockTags.create(new ResourceLocation("unusualend:citrine_totem_base")))
			...
			for (int index0 = 0; index0 < (int) (double) ConfigurationFileConfiguration.CITRINE_TOTEM_ACCURACY.get(); index0++) {
				...
				if ((world.getBlockState(BlockPos.containing(X, Y, Z))).getBlock() instanceof BonemealableBlock) {
					...
					if (world instanceof Level _level) {
						if (!_level.isClientSide()) {
							_level.playSound(null, BlockPos.containing(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("item.bone_meal.use")), SoundSource.BLOCKS, (float) 1.5,
									(float) Mth.nextDouble(RandomSource.create(), 0.5, 0.7));
						} else {
							_level.playLocalSound(x, y, z, ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("item.bone_meal.use")), SoundSource.BLOCKS, (float) 1.5, (float) Mth.nextDouble(RandomSource.create(), 0.5, 0.7), false);
						}
					}
					dividedby = 1;
					for (int index1 = 0; index1 < 15; index1++) {
						...
						if (world instanceof ServerLevel _level)
							_level.sendParticles((SimpleParticleType) (UnusualendModParticleTypes.CITRINE_SHINE.get()), (x + 0.5 + (X - (x + 0.5)) / dividedby + Mth.nextDouble(RandomSource.create(), -0.2, 0.2)),
