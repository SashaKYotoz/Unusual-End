...
public class UnbucketGlubProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, ItemStack itemstack) {
		...
		if (!entity.isShiftKeyDown()) {
			...
			if (itemstack.getOrCreateTag().getBoolean("isBaby")) {
				if (world instanceof ServerLevel _level)
					_level.getServer().getCommands().performPrefixedCommand(
							...
							"summon unusualend:glub ~ ~ ~ {Age:-6000}");
			} else {
				if (world instanceof ServerLevel _level) {
					Entity entityToSpawn = UnusualendModEntities.GLUB.get().spawn(_level, BlockPos.containing(x + 0.5, y + 1, z + 0.5), MobSpawnType.MOB_SUMMONED);
			...
			if (!world.getEntitiesOfClass(WarpedJellyfishEntity.class, AABB.ofSize(new Vec3((x + 0.5), (y + 1), (z + 0.5)), 1, 1, 1), e -> true).isEmpty() && itemstack.getOrCreateTag().getBoolean("isNamed")) {
				name = itemstack.getDisplayName().getString();
				name = name.substring(1, (int) ((name).length() - 1));
				((Entity) world.getEntitiesOfClass(WarpedJellyfishEntity.class, AABB.ofSize(new Vec3((x + 0.5), (y + 1), (z + 0.5)), 1, 1, 1), e -> true).stream().sorted(new Object() {
			...
			if (itemstack.getOrCreateTag().getDouble("tagHealth") != 0) {
				if (((Entity) world.getEntitiesOfClass(WarpedJellyfishEntity.class, AABB.ofSize(new Vec3((x + 0.5), (y + 1), (z + 0.5)), 1, 1, 1), e -> true).stream().sorted(new Object() {
					Comparator<Entity> compareDistOf(double _x, double _y, double _z) {
						return Comparator.comparingDouble(_entcnd -> _entcnd.distanceToSqr(_x, _y, _z));
					}
