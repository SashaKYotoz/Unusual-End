...
public class CelestialFluidRendererProcedure {
	...
	private static boolean begin(VertexFormat.Mode mode, VertexFormat format, boolean update) {
		if (CelestialFluidRendererProcedure.bufferBuilder == null || !CelestialFluidRendererProcedure.bufferBuilder.building()) {
			if (update)
				clear();
			if (CelestialFluidRendererProcedure.vertexBuffer == null) {
				if (format == DefaultVertexFormat.POSITION_COLOR) {
					CelestialFluidRendererProcedure.mode = mode;
					CelestialFluidRendererProcedure.format = format;
					CelestialFluidRendererProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					CelestialFluidRendererProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_COLOR);
					return true;
				} else if (format == DefaultVertexFormat.POSITION_TEX_COLOR) {
					CelestialFluidRendererProcedure.mode = mode;
					CelestialFluidRendererProcedure.format = format;
					CelestialFluidRendererProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					CelestialFluidRendererProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_TEX_COLOR);
					return true;
				}
			}
		}
	...
	private static void system(boolean worldCoordinate) {
		CelestialFluidRendererProcedure.worldCoordinate = worldCoordinate;
	}

	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			CelestialFluidRendererProcedure.targetStage = targetStage;
			return true;
		}
		return false;
	}
